...
const FileInput = ({setFile}:IFileInput) => {
    return ( 
        <div className="flex items-center justify-center w-full mt-12">
            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-yellow-600 border-dashed rounded-lg cursor-pointer hover:bg-black bg-bitBrown">
                <div className="h-32 flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="mb-2 text-sm text-yellow-600 font-semibold">Click to upload document</p>
                    <p className="text-xs text-yellow-600">PDF</p>
                </div>
                <input id="dropzone-file" type="file" accept="application/pdf" className="hidden" onChange={(e)=>setFile(e)} />
            </label>
        </div>
     );
}
